...
### ✅ ЗАДАЧА 4 — ВЫПОЛНЕНО 12.06.2026 (расширенно)
`src/keyboard.ts`: интерактивная SVG-копия схемы оригинала 1:1 (включая 29 стрелок направления движения пальцев — требование Дениса). Подсветка следующей клавиши + анимация стрелки маршрута от домашней клавиши + Shift противоположной руки. RU/EN маппинг символов.

### ✅ ЭТАПЫ 2–3 (три версии приложения) — ВЫПОЛНЕНО 12.06.2026
- **Профили М/Ж/дети** (`src/profiles.ts`): онбординг с тремя карточками при первом запуске, селект в шапке, темы через `[data-profile]` (Ж — кремово-розовая по гамме ageeva-site; решение «один код — три профиля», НЕ три репо).
- **Детский режим** (`src/kids.ts`): 18 уровней (9 RU + 9 EN, банк 90 слов 3–5 букв), уровень = 5 слов, звёзды 1–3 по точности (0 ошибок = 3⭐), котик-маскот, карта с замками, прогресс `tr_kids`, «передохни» после 3 уровней, звуки Web Audio. Скорость детям не показывается.
- **Мост раскладок** (`bridgeChar` в keyboard.ts): физическая клавиша важнее раскладки ОС — везде, и у взрослых.
- Осталось опц.: раздельные бандлы трёх версий (этап 4), интерактив-доработки по фидбеку Алекса.

### ✅ Режим «Поток» (запрос Дениса 12.06) — v2.4.0
Как Stamina: строки сшиты (без Enter), упражнения подряд без экрана «Готово», сквозная статистика серии + счётчик 🔥. Построчный режим остался, переключатель «Поток» в тулбаре.
