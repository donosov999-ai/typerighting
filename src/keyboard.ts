/* typerighting-layouts · VER 18 · 17.09.2026 */
// Интерактивная SVG-клавиатура — векторная копия схемы из оригинального
// TypeRIGHT (public/images/keyboard.webp, 920×380): раскладка ЙЦУКЕН↔QWERTY,
...
// Снято с public/images/keyboard.webp. Цифровой ряд в оригинале покрыт
...
// ── Цвета пальцев (раскраска зон одноручной печати) ──
const FINGER_COLOR: Record<string, string> = {
  lp: '244,114,182', lr: '251,146,60', lm: '250,204,21', li: '74,222,128',
  ri: '34,211,238', rm: '96,165,250', rr: '167,139,250', rp: '236,72,153',
};

// ── Одноручная аппликатура (центральная постановка, рука печатает ВСЮ зону) ──
// Денис 13.06: правая рука — указатель на А, мизинец на О; левая — зеркально.
// 4 группы столбцов A/B/C/D (лево→право). Сильные пальцы (указат./мизинец)
// берут края, средний/безымянный — центр.
const COL_GROUP: Record<string, 'A' | 'B' | 'C' | 'D'> = {
  // A — центр-лево + левый край (вкл А=F)
  tilde: 'A', d1: 'A', d2: 'A', d3: 'A', d4: 'A', tab: 'A', caps: 'A', lshift: 'A',
  q: 'A', w: 'A', e: 'A', r: 'A', a: 'A', s: 'A', d: 'A', f: 'A', z: 'A', x: 'A', c: 'A', v: 'A',
  // B — столбец П=G
  d5: 'B', t: 'B', g: 'B', b: 'B',
  // C — столбец Р=H
  d6: 'C', y: 'C', h: 'C', n: 'C',
  // D — центр-право + правый край (вкл О=J)
  d7: 'D', d8: 'D', d9: 'D', d0: 'D', minus: 'D', equal: 'D', backslash: 'D', backspace: 'D',
  u: 'D', i: 'D', o: 'D', p: 'D', lbracket: 'D', rbracket: 'D', enter: 'D',
  j: 'D', k: 'D', l: 'D', semi: 'D', quote: 'D', m: 'D', comma: 'D', period: 'D', slash: 'D', rshift: 'D', rept: 'D',
};
// группа → палец (цвет): правая рука vs левая (зеркально)
const GROUP_FINGER_R: Record<string, string> = { A: 'ri', B: 'rm', C: 'rr', D: 'rp' };
const GROUP_FINGER_L: Record<string, string> = { A: 'lp', B: 'lr', C: 'lm', D: 'li' };
...
export function keyboardSVG(nextChar: string | null, ruContext: boolean, showRu = true, heat: Record<string, number> | null = null, handZone: 'left' | 'right' | null = null): string {
  ...
  for (const g of Object.values(GEO)) {
    ...
    // одноручная раскраска: вся зона по пальцам выбранной руки (центральная постановка)
    if (handZone) {
      const grp = COL_GROUP[g.id];
      if (grp) {
        const fg = (handZone === 'right' ? GROUP_FINGER_R : GROUP_FINGER_L)[grp];
        parts.push(`<rect x="${fx}" y="${fy}" width="${fw}" height="${fh}" rx="6" fill="rgb(${FINGER_COLOR[fg]})" opacity="0.42"/>`);
      }
    }
